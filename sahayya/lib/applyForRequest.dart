...
class ApplyForRequest extends StatefulWidget {
  const ApplyForRequest({Key? key}) : super(key: key);

  @override
  _ApplyForRequestState createState() => _ApplyForRequestState();
}

class _ApplyForRequestState extends State<ApplyForRequest> {
              ...
              ElevatedButton(
                onPressed: () async {
                  ...
                  final response = await http.post(
                      Uri.parse(
                          'https://asia-south1-sahayya-9c930.cloudfunctions.net/api/apply-to-donor-give-out'),
                  ...
                  if(response.statusCode == 201){
                    final snackBar = SnackBar(
                      content: Text('Applied Successfully for the NGO request.'),
                    );
                    ScaffoldMessenger.of(context).showSnackBar(snackBar);
                    if(TYPE == 'Company'){
                  ...
                  if(response.statusCode == 403){
                    final snackBar = SnackBar(
                      content: Text('You have already applied for NGO request.'),
                    );
                    ScaffoldMessenger.of(context).showSnackBar(snackBar);
                    return;
                  }
